...
const DoubleDateIcon = () => (
  <div className="flex items-center justify-center w-12 h-12 bg-sky-900/30 rounded-full">
    <Heart size={24} fill="currentColor" className="text-sky-400" />
  </div>
);
...
const BrandLogo = ({ type, textColor = "text-white" }) => (
  <div className="flex items-baseline space-x-2">
    ...
    <span className={`text-[8px] font-bold px-2 py-0.5 rounded-sm uppercase ${type === 'Gold' ? 'bg-black text-yellow-500' : type === 'Platinum' ? 'bg-white text-black' : 'bg-sky-500 text-white'}`}>
      {type}
    </span>
  </div>
);
...
const ProfileHeader = ({ userData, onNavigate, onSignOut, db }) => {
  ...
  const getBadge = () => {
    ...
    if (tier === 'weekly') return <CheckCircle2 size={20} className="text-white fill-sky-500 ml-1" />;
    if (tier === 'gold') return <CheckCircle2 size={20} className="text-black fill-yellow-400 ml-1" />;
    if (tier === 'platinum') return <CheckCircle2 size={20} className="text-white fill-zinc-500 ml-1" />;
    return null; 
  };
    ...
    <div className="flex items-center w-full px-4 pt-6">
      <div className="mr-4 relative w-24 h-24 flex-shrink-0">
        <svg className="w-full h-full rotate-[-90deg]" viewBox="0 0 100 100">
          ...
          <circle className="text-sky-500 transition-all duration-1000 ease-out" strokeWidth="6" strokeDasharray="289" strokeDashoffset={289 - (percentage / 100) * 289} strokeLinecap="round" stroke="currentColor" fill="transparent" r="46" cx="50" cy="50" />
        </svg>
        <div className="absolute inset-0 p-1.5 rounded-full">
          <label className={`w-full h-full rounded-full flex items-center justify-center overflow-hidden cursor-pointer relative group bg-zinc-900 shadow-inner ${!hasImage ? 'bg-sky-900/20 border border-dashed border-sky-800' : ''}`}>
            ...
            {uploading && <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10"><Loader className="animate-spin text-white" size={24} /></div>}
            {hasImage ? <img className="w-full h-full rounded-full object-cover" src={displayPhoto} alt="Profile" onError={(e) => e.target.src = placeholder} /> : <Plus size={32} className="text-sky-400" />}
          </label>
        </div>
      </div>
      <div className="flex flex-col items-start z-10">
        ...
        <button onClick={() => onNavigate('edit')} className="flex items-center cursor-pointer justify-center px-4 py-2 mt-2 space-x-2 text-sm font-semibold text-black bg-sky-400 rounded-full shadow-md hover:bg-sky-300 transition-colors">
          <Pencil size={14} /><span>Edit profile</span>
        </button>
      </div>
      <div className="ml-auto block md:hidden">
        <button onClick={onSignOut} className="text-zinc-500 hover:text-red-500 transition-colors p-2" title="Sign Out"><LogOut size={24} /></button>
      </div>
    </div>
  );
};
...
const DoubleDateBanner = ({ user }) => {
  ...
  const handleNativeShare = async () => {
    if (navigator.share) { try { await navigator.share({ title: 'BSSS Dating', text: inviteText, url: appUrl }); } catch (error) { console.log('Error sharing:', error); } }
    else { try { await navigator.clipboard.writeText(inviteText); alert("Invite link copied to clipboard!"); } catch (err) { alert("Unable to copy link"); } }
  };
  return (
    <div className="px-4 mt-6">
      <div onClick={handleNativeShare} className="flex items-center justify-between cursor-pointer p-4 bg-zinc-900 border border-white/5 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 group">
        <div className="flex items-center space-x-4">
          <DoubleDateIcon />
          <div><h2 className="font-bold text-white group-hover:text-sky-400 transition-colors">Try Double Date</h2><p className="text-sm text-zinc-400">Invite friends & find pairs.</p></div>
        </div>
...
const ActionCard = ({ icon, title, subtitle, subtitleColor, onClick }) => (
  <div onClick={onClick} className="relative flex flex-col items-center justify-center h-32 p-3 text-center bg-zinc-900 border border-white/5 rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer active:scale-95 duration-150">
    <div className="absolute top-2 right-2 flex items-center justify-center w-5 h-5 bg-zinc-800 rounded-full"><Plus size={14} className="text-sky-400" /></div>
    <div className="mt-2">{icon}</div>
    <h3 className="mt-2 text-sm font-semibold text-white">{title}</h3>
    {subtitle && <p className={`mt-1 text-xs font-bold ${subtitleColor}`}>{subtitle}</p>}
  </div>
);
...
const FeatureRow = ({ title, textColor, currentType, freeFeatures }) => {
  ...
  return (
    <div className="grid grid-cols-3 items-center mt-3 text-center border-b border-white/5 pb-2 last:border-0">
      <span className={`text-[11px] xs:text-sm font-medium text-left truncate pr-1 ${textColor}`}>{title}</span>
      <span className={`flex justify-center ${textColor} opacity-40`}>{isFreeFeature ? <Check size={18} /> : <Lock size={16} />}</span>
      <span className={`flex justify-center ${textColor}`}><Check size={18} /></span>
    </div>
  );
};
  ...
  <div className="min-w-full px-4 box-border snap-center md:min-w-0 md:px-0 flex flex-col">
    <div className={`p-5 rounded-[24px] shadow-lg bg-gradient-to-br ${data.gradient} flex flex-col flex-1 min-h-[280px]`}>
      <div className="flex items-center justify-between mb-6">
        ...
        {/* Pass the FULL data object (including price) to onUpgrade */}
        <button onClick={() => !isCurrent && onUpgrade && onUpgrade(data)} disabled={isCurrent} className={`px-6 py-2 font-bold rounded-full shadow-md text-sm transition-all ${isCurrent ? (data.type === 'Gold' ? "bg-black text-yellow-400 cursor-default" : "bg-white text-black cursor-default") : `${data.btnColor} ${data.btnTextColor} hover:opacity-90 cursor-pointer active:scale-95`}`}>
      ...
      <div className={`flex-1 ${data.textColor}`}>
        <div className="grid grid-cols-3 text-[10px] uppercase tracking-wider font-bold text-center mb-4 border-b border-current/10 pb-2">
          <span className="text-left">Plan Perks</span><span className="opacity-60">Free</span><span className="opacity-100">{data.type}</span>
        </div>
        <div className="flex flex-col justify-start">
          {data.features.map((feature, index) => <FeatureRow key={index} title={feature} textColor={data.textColor} currentType={data.type} freeFeatures={freeFeatures} />)}
        </div>
      </div>
      <p className={`text-center text-[10px] mt-4 opacity-60 font-medium ${data.textColor}`}>Cancel anytime • Secure Payment</p>
    </div>
  </div>
);
...
const UpgradeCarousel = ({ onUpgrade, currentTier }) => {
  ...
  return (
    <div className="mt-8">
      ...
      <div ref={scrollRef} onScroll={handleScroll} className="flex overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-4 md:grid md:grid-cols-3 md:gap-4 md:px-4 md:overflow-visible" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        {tiers.map((tier, index) => <UpgradeCard key={index} data={tier} freeFeatures={freeFeatures} onUpgrade={onUpgrade} isCurrent={currentTier?.toLowerCase() === tier.type.toLowerCase()} />)}
      </div>
      <div className="flex justify-center w-full space-x-1.5 md:hidden">
        {tiers.map((_, index) => <div key={index} className={`w-2 h-2 rounded-full transition-colors duration-300 ${activeIndex === index ? 'bg-sky-400' : 'bg-zinc-700'}`}></div>)}
      </div>
    </div>
  );
};
...
const AuthorSection = ({ onOpenPopup }) => (
  <div className="px-4 mt-8 mb-4">
    <button onClick={onOpenPopup} className="w-full p-3 bg-zinc-900 border border-white/5 cursor-pointer rounded-lg shadow-sm hover:shadow-md transition-all flex items-center justify-between group">
      <div className="flex items-center space-x-3">
        ...
        <div className="text-left"><p className="text-xs text-zinc-500 font-medium">Created by</p><p className="text-sm font-bold text-white">Anonymous</p></div>
      </div>
  ...
  <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm" onClick={onClose}>
    <div className="bg-zinc-900 border border-white/10 rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden transform transition-all" onClick={(e) => e.stopPropagation()}>
      <div className="relative h-40 bg-gradient-to-br from-sky-400 via-blue-600 to-sky-900 flex items-center justify-center">
        <button onClick={onClose} className="absolute top-4 cursor-pointer right-4 w-10 h-10 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center hover:bg-white/20 transition-all text-white"><X size={20} /></button>
      ...
      <div className="p-6">
        <div className="text-center mb-6"><h2 className="text-2xl font-bold text-white">Anonymous</h2><p className="text-sm text-zinc-500 mt-1">Developer</p></div>
        <div className="space-y-4">
          <div className="flex items-start space-x-3 bg-white/5 p-4 rounded-xl">
            <div className="w-10 h-10 bg-gradient-to-br from-sky-400 to-blue-600 rounded-lg flex items-center justify-center flex-shrink-0 shadow-md"><GraduationCap className="text-white w-6 h-6" /></div>
            <div><p className="text-sm font-semibold text-white">Professional</p><p className="text-xs text-zinc-400 mt-1">Studying in BSSS College 3rd year in 6th Semester</p></div>
          </div>
          <a href="https://www.instagram.com/bsssdating?igsh=MW92bmpja2M3N2ZnOA%3D%3D&utm_source=qr" target="_blank" rel="noopener noreferrer" className="flex items-start space-x-3 bg-white/5 p-4 rounded-xl hover:bg-sky-500/10 transition-colors cursor-pointer group">
            <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 shadow-md group-hover:shadow-lg transition-all bg-gradient-to-tr from-[#F58529] via-[#DD2A7B] to-[#8134AF]"><Instagram className="text-white w-6 h-6" /></div>
            <div><p className="text-sm font-semibold text-white">Instagram</p><p className="text-xs text-zinc-400 mt-1">Follow - @bsssdating</p></div>
          </a>
        </div>
...
const ProfileScreen = ({ onNavigate, userData, onSignOut, db, onUpgrade }) => {
  ...
  return (
    <div className="flex flex-col pb-4 bg-black">
      ...
      {/* We pass the whole onUpgrade function which receives the plan object */}
...
export default function Profile() {
  ...
  // React Router Hook for external navigation
  ...
  useEffect(() => {
    if (dbInstance && userId && user) {
      ...
      return () => { unsubArtifacts(); unsubUsers(); };
    }
  ...
  // --- NAVIGATION HANDLER ---
  // Handles 'edit' (internal state) and 'premium' (external route with state)
  const handleNavigation = (target, params) => {
      if (target === 'premium') {
          // params example: { type: 'Gold', price: 199 }
          routerNavigate('/premium', { state: params });
      } else {
          navigate(target);
      }
  };
      ...
      <div className="w-full max-w-sm bg-black rounded-2xl shadow-xl sm:max-w-[95%] border border-white/5 overflow-hidden">
        {currentView === 'profile' && (
           <ProfileScreen 
              ...
              db={dbInstance} 
              onUpgrade={(planData) => handleNavigation('premium', planData)} 
           />
        )}
        {currentView === 'edit' && (
           <EditProfile 
